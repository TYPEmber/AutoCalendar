...
public class AutoExpandLinearLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int bufferMax = 0;
        int containerHeight = 0;// 容器的高度,也就是本布局的高度。初始化赋值为0.
        ...
        setMeasuredDimension(maxWidth, containerHeight);// onMeasure方法的关键代码,该句设置父容器的大小。
    }
    @Override
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if(text.equals("")) {//用于处理换行符问题
                row++;
                left = 0;
                right = 0;
                continue;
            }
            if (right + p/2 > maxWidth) {
                row++;
                left = 0;
                right = 0;
                left = p + right ;
